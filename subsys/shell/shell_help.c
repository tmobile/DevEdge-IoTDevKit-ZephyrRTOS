...
void z_shell_help_subcmd_print_selitem(const struct shell *sh)
{
	struct shell_static_entry *parent = &sh->ctx->active_cmd;

	help_item_print(sh, parent->syntax, strlen(parent->syntax), parent->help);
}
