...
#else
		ret = tsl2540_reg_read(dev, TSL2540_REG_VIS_LOW, (uint8_t *)&data->count_vis);
		if (ret < 0) {
			LOG_ERR("Could not fetch ambient light (visible)");
		}
		ret = tsl2540_reg_read(dev, TSL2540_REG_VIS_HI, ((uint8_t *)&data->count_vis) + 1);
		if (ret < 0) {
			LOG_ERR("Could not fetch ambient light (visible)");
		}
		data->count_vis = sys_le16_to_cpu(data->count_vis);
#endif
	}
...
#else
		ret = tsl2540_reg_read(dev, TSL2540_REG_IR_LOW, (uint8_t *)&data->count_ir);
		if (ret < 0) {
			LOG_ERR("Could not fetch ambient light (IR)");
		}
		ret = tsl2540_reg_read(dev, TSL2540_REG_IR_HI, ((uint8_t *)&data->count_ir) + 1);
		if (ret < 0) {
			LOG_ERR("Could not fetch ambient light (IR)");
		}
		data->count_ir = sys_le16_to_cpu(data->count_ir);
#endif
	}
	k_sem_give(&data->sem);
...
#else
	tsl2540_attr_set(dev, SENSOR_CHAN_LIGHT, SENSOR_ATTR_GAIN,
			 &(struct sensor_value){.val1 = TSL2540_SENSOR_GAIN_16, .val2 = 0});
#endif
...
static int tsl2540_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_RESUME:
		reg = tsl2540_reg_write(dev, TSL2540_ENABLE_ADDR,
