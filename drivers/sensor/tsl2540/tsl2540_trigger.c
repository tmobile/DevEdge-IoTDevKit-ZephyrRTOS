...
static void tsl2540_handle_int(const struct device *dev)
{
	...
	if ((ret = tsl2540_reg_read(dev, TSL2540_REG_STATUS, &status))) {
			...
			sensor_trigger_handler_t handler)
{
	...
	if (SENSOR_CHAN_LIGHT == trig->chan) {
		switch (trig->type) {
		case SENSOR_TRIG_THRESHOLD: {
			const struct i2c_dt_spec *i2c_spec =
				&((const struct tsl2540_config *)dev->config)->i2c_spec;

			if ((ret = i2c_reg_update_byte_dt(i2c_spec, TSL2540_INTENAB_ADDR,
							  TSL2540_INTENAB_MASK,
							  TSL2540_INTENAB_CONF))) {
				LOG_ERR("%#x: I/O error: %d", TSL2540_INTENAB_ADDR, ret);
			} else if ((ret = i2c_reg_update_byte_dt(i2c_spec, TSL2540_CFG3_ADDR,
								 TSL2540_CFG3_MASK,
								 TSL2540_CFG3_CONF))) {
				LOG_ERR("%#x: I/O error: %d", TSL2540_CFG3_ADDR, ret);
				/* } else if ((ret = i2c_reg_update_byte_dt(i2c_spec,
				   TSL2540_ENABLE_ADDR, TSL2540_ENABLE_MASK, TSL2540_ENABLE_CONF)))
				   { LOG_ERR("%#x: I/O error: %d", TSL2540_ENABLE_ADDR, ret); */
			} else {
				struct tsl2540_data *data = dev->data;
