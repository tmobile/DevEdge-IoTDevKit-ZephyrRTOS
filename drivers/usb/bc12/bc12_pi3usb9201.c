...
static int pi3usb9201_set_mode(const struct device *dev, enum pi3usb9201_mode desired_mode)
{
	const struct pi3usb9201_config *cfg = dev->config;
...
static int pi3usb9201_get_status(const struct device *dev, uint8_t *client, uint8_t *host)
{
	const struct pi3usb9201_config *cfg = dev->config;
	int rv;
	uint8_t status;
...
static void pi3usb9201_notify_callback(const struct device *dev, struct bc12_partner_state *state)
{
...
static void pi3usb9201_client_detect_finish(const struct device *dev, int client_status)
{
	...
	LOG_DBG("client_sts = 0x%x, current = %d mA, type = %d", client_status, new_chg.current_ua,
		new_chg.type);
...
int pi3usb9201_set_result_cb(const struct device *dev, bc12_callback_t cb, void *user_data)
{
	struct pi3usb9201_data *pi3usb9201_data = dev->data;
