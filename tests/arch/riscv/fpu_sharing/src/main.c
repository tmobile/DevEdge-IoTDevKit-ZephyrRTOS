...
void test_fp_insn_trap(void)
{
	...
	/* RVC variants */
#if defined(CONFIG_COMPRESSED_ISA)
...
#endif
#endif /* CONFIG_COMPRESSED_ISA */
}

void test_main(void)
{
	ztest_test_suite(riscv_fpu_sharing,
			 ztest_unit_test(test_basics),
			 ztest_unit_test(test_multi_thread_interaction),
			 ztest_unit_test(test_thread_vs_exc_interaction),
			 ztest_unit_test(test_fp_insn_trap));
	ztest_run_test_suite(riscv_fpu_sharing);
}
